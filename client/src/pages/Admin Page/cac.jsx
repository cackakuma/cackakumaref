...
const CAC = () => {
  ...
  // Integrity handlers
  const addIntegrity = () => setIntegrity([...integrity, { impact: "", details: "" }]);
  const removeIntegrity = () => integrity.length > 0 && setIntegrity(integrity.slice(0, -1));
  const updateIntegrityField = (index, field, value) => {
    const next = [...integrity];
    next[index] = { ...next[index], [field]: value };
    setIntegrity(next);
  };
  const deleteIntegrity = (index) => setIntegrity(integrity.filter((_, i) => i !== index));
  ...
  const handleSubmit = async (e) => {
    ...
    if (!orgName.trim() || !acronym.trim() || !description.trim() || !mission.trim() || !vision.trim()) {
    ...
    try {
      if (editingId) {
        ...
        setOrgs((prev) => prev.map((org) => (org._id === editingId ? updatedOrg : org)));
    ...
    <div className="max-w-7xl mx-auto px-6 py-10 relative">
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6">
          <h1 className="text-white text-3xl font-bold text-center">Organisation Management</h1>
          <p className="text-blue-100 text-center mt-2">Manage CAC Organisation and its information</p>
        </div>
      </div>

      <div className="flex justify-center gap-4 mb-8">
        <button
          onClick={orgs.length > 0 ? () => startEdit(orgs[0]) : () => { resetForm(); setView("form"); }}
          className={`px-8 py-3 font-semibold rounded-xl shadow-lg flex items-center space-x-2 ${
            view === "form" ? "bg-blue-600 text-white transform scale-105" : "bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-50"
          }`}
        >
          <span>{editingId ? "Edit Organisation" : "Add Organisation"}</span>
        </button>

        <button
          onClick={() => setView("list")}
          className={`px-8 py-3 font-semibold rounded-xl shadow-lg flex items-center space-x-2 ${
            view === "list" ? "bg-blue-600 text-white transform scale-105" : "bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-50"
          }`}
        >
      ...
      {view === "form" && (
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl mx-auto">
            {[
              { label: "Official Name", value: orgName, onChange: (e) => setOrgName(e.target.value), placeholder: "Enter Official Name" },
              { label: "Acronym", value: acronym, onChange: (e) => setAcronym(e.target.value.toUpperCase()), placeholder: "ABC" },
              { label: "Organisation Bio", value: description, onChange: (e) => setDescription(e.target.value), placeholder: "Organisation description" },
              { label: "Mission", value: mission, onChange: (e) => setMission(e.target.value), placeholder: "Mission statement" },
              { label: "Vision", value: vision, onChange: (e) => setVision(e.target.value), placeholder: "Vision statement" },
              { label: "Objective", value: objective, onChange: (e) => setObjective(e.target.value), placeholder: "Objective" },
            ...
            <div className="flex gap-4">
              <button type="submit" className="flex-grow bg-blue-700 text-white py-3 rounded-xl">{editingId ? "Update Organisation" : "Add Organisation"}</button>
      ...
      {view === "list" && (
        <div className="space-y-6">
          ...
          {!loading && !error && orgs.length === 0 && <div className="bg-white p-8 rounded-xl text-center shadow"><p className="text-gray-600 font-semibold">No data found</p></div>}

          {!loading && !error && orgs.length > 0 && (
            <>
              <div className="bg-white rounded-xl shadow-lg p-6 text-center">
                <h3 className="text-2xl font-bold text-gray-800">Total Organisations: {orgs.length}</h3>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {orgs.map((org) => (
                  <div key={org._id} className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
                    <div className="flex justify-between">
                      <h2 className="text-xl font-bold">{org.orgName}</h2>
                      <div className="flex gap-2">
                        <button onClick={() => startEdit(org)} className="bg-blue-600 text-white px-3 py-2 rounded-lg">Edit</button>
                        <button onClick={() => handleDelete(org._id)} className="bg-red-600 text-white px-3 py-2 rounded-lg">Delete</button>
                      </div>
                    </div>
                    {org.logo?.length > 0 && <img src={org.logo[0].link} className="w-48 h-48 mt-4 rounded-lg" />}
                    <p className="mt-2 text-gray-700">{org.description}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                      <div className="bg-gray-50 rounded-lg p-3"><span className="font-medium text-gray-600">Mission:</span><p>{org.mission}</p></div>
                      <div className="bg-gray-50 rounded-lg p-3"><span className="font-medium text-gray-600">Vision:</span><p>{org.vision}</p></div>
                      <div className="bg-gray-50 rounded-lg p-3"><span className="font-medium text-gray-600">Objective:</span><p>{org.objective}</p></div>
                      {org.integrity?.length > 0 && (
