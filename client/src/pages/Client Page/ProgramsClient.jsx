...
const Programs = () => {
  ...
  const renderProgram = (p) => {
    ...
    const benefits = Array.isArray(p.programBenefits)
      ? p.programBenefits.map(b => (typeof b === "string" ? b : (b?.benefit || ""))).filter(Boolean)
      : [];
    const impacts = Array.isArray(p.communityImpact)
      ? p.communityImpact.map(ci => ({ impact: ci?.impact || "", desc: ci?.details || "" })).filter(x => x.impact || x.desc)
      : [];
  ...
  return(
    <div className="relative w-full min-h-screen">
      ...
      <div className="pt-28">
        ...
        >
          <Routes>
            <Route index element={<Navigate to={firstSlug || 'more'} replace />} />
            {programs.map((p) => (
              <Route key={p._id} path={slugify(p.title)} element={renderProgram(p)} />
            ))}
